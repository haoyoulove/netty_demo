...
//
//
//public class ConcurrentHashMap8<K,V> extends AbstractMap<K,V>
...
//     * HASH_BITS = Integer.MAX_VALUE = 0x7fffffff = 2147483647 = 1111111111111111111111111111111
//     *
//     */
...
//     * 基本计数器值，主要用于没有争用时，也用作表初始化竞赛期间的回退。 通过CAS更新。
//     */
...
//        11 + 5 + 1 = 17
//
//        1.0 + (long)initialCapacity / loadFactor
//        1 + 13
//
//
//     */
...
//                // 初始化table数组
//                tab = initTable();
//                // 硬件级别的原子获取hash值对应的table数组元素
//            } else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {
//                //在这个元素上节点是null，那么就可以直接创建节点设置在这个位置上，
//                //考虑同步问题，使用CAS机制来比较修改.如果修改失败，就不会break,重新进入循环即可.(这个时候可能这个位置有值了)
...
//                if (binCount != 0) {
//                    // 判断是否要将链表转换为红黑树，临界值和 HashMap 一样，也是 8 上面就算走红黑树，binCount也只是=2,,所以不会走这里
...
//            (new ConcurrentHashMap8<K,Boolean>(), Boolean.TRUE);
//    }
//
//    /**
//     * Creates a new {@link Set} backed by a ConcurrentHashMap
...
//        return new KeySetView<K,Boolean>
//            (new ConcurrentHashMap8<K,Boolean>(initialCapacity), Boolean.TRUE);
//    }
//
//    /**
...
//            //如果sizeCtl已经被初始化获得值了，例如sizeCtl=16,那么sc=16
//            if ((sc = sizeCtl) < 0) {
//                // 初始化的"功劳"被其他线程"抢去"了
//                Thread.yield(); // lost initialization race; just spin
//                // CAS 一下，将 sizeCtl 设置为 -1，代表抢到了锁 (SIZECTL是sizeCtl类中的偏移量)，如果上面sc被设置值了，这里没有改变sc。
//            } else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
//                try {
//                    if ((tab = table) == null || tab.length == 0) {
//                        // DEFAULT_CAPACITY 默认初始容量是 16， sc可能是sizeCtl提供的长度
...
//				if (--i >= bound || finishing) { // 经过测试 i 是先减1 再比较的
//					advance = false;
//
...
//				advance = casTabAt(tab, i, null, fwd); //放入老table里面,标记完成这个位置的处理了。
...
//     如果table太小，选择扩容大小。要不然就替换掉所有链表结点.树形化
//     */
...
//         */
//        //将treeNode这种双向链表彻底变成红黑树
//        TreeBin(TreeNode<K,V> b) {
//            super(TREEBIN, null, null, null);
//            this.first = b;  //treenode第一个节点
//            TreeNode<K,V> r = null;
//            //遍历treenode
//            for (TreeNode<K,V> x = b, next; x != null; x = next) {
//                next = (TreeNode<K,V>)x.next;
//                x.left = x.right = null;
//                //第一次赋值，头节点就是根结点，根结点是黑树
//                if (r == null) {
//                    x.parent = null;
//                    x.red = false;
//                    r = x;
...
//                    int h = x.hash;
//                    Class<?> kc = null;
//                    for (TreeNode<K,V> p = r;;) { //从根结点开始遍历现有的节点
//                        int dir, ph;
//                        K pk = p.key;
//                        // x比当前遍历结点小
//                        if ((ph = p.hash) > h) {
...
//                            //x和当前遍历结点相同
//
//                            // key如果实现comparable接口调用compareComparables来比较。
//                        } else if ((kc == null && (kc = comparableClassFor(k)) == null)  //如果key没有实现Comparable接口，调用 tieBreakOrder
//                                ||  (dir = compareComparables(kc, k, pk)) == 0) { //如果compareTo比较仍然等于0,调用 tieBreakOrder 计算dir值
//                            dir = tieBreakOrder(k, pk);  // 最终比较key对象的默认hashCode()方法的返回值
//                        }
//                        TreeNode<K,V> xp = p; //当前遍历节点赋予xp
//
//                        if ((p = (dir <= 0) ? p.left : p.right) == null) {
//                            // dir已经判断出x要插在左还是右,判断当前遍历要插入位置是空
//                            x.parent = xp; //插入节点的父节点是当前遍历节点
//                            if (dir <= 0) {  //插入左节点
//                                xp.left = x;
//                            } else { // 插入右节点
//                                xp.right = x;
//                            }
//                            //加入的节点有可能破坏红黑树特性，调整二叉树来重新满足特性。也就是一些自旋、变色操作，后面再看
...
//                         long est, ConcurrentHashMap8<K,V> map) {
//            super(tab, size, index, limit);
//            this.map = map;
//            this.est = est;
//        }
...
//        private static final long serialVersionUID = 7249069246763182397L;
//        final ConcurrentHashMap8<K,V> map;
//        CollectionView(ConcurrentHashMap8<K,V> map)  { this.map = map; }
//
//        /**
...
//         */
//        public ConcurrentHashMap8<K,V> getMap() { return map; }
//
//        /**
...
//        KeySetView(ConcurrentHashMap8<K,V> map, V value) {  // non-public
//            super(map);
//            this.value = value;
//        }
//
...
//        private static final long serialVersionUID = 2249069246763182397L;
//        ValuesView(ConcurrentHashMap8<K,V> map) { super(map); }
//        @Override
...
//        private static final long serialVersionUID = 2249069246763182397L;
//        EntrySetView(ConcurrentHashMap8<K,V> map) { super(map); }
//
//        @Override
